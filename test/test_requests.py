...
def test_deletion_autorequest(client: TestClient, pm_user: User, pkgbase: PackageBase) -> None:
...
def test_orphan_as_maintainer(client: TestClient, auser: User, pkgbase: PackageBase) -> None:
    endpoint = f"/pkgbase/{pkgbase.Name}/disown"
    data = {"confirm": True}
    with client as request:
        request.cookies = auser.cookies
...
def test_closure_factory_invalid_reqtype_id() -> None:
...
def test_pkgreq_by_id_not_found() -> None:
    with pytest.raises(HTTPException):
        get_pkgreq_by_id(0)


def test_requests_unauthorized(client: TestClient) -> None:
    with client as request:
...
def test_requests_close(client: TestClient, user: User, pkgreq: PackageRequest) -> None:
    cookies = {"AURSID": user.login(Request(), "testPassword")}
    with client as request:
        request.cookies = cookies
...
def test_requests_close_post(client: TestClient, user: User, pkgreq: PackageRequest) -> None:
    cookies = {"AURSID": user.login(Request(), "testPassword")}
    with client as request:
        request.cookies = cookies
